...
bool interpolateAndDisplay(float t) {
	...
	vector<RigTForm>::iterator r1iter = rtf1.begin();
	vector<RigTForm>::iterator r2iter = rtf2.begin();

	for(; r1iter != rtf1.end(), r2iter != rtf2.end(); ++r1iter, ++r2iter) {
		(*niter)->setRbt(lerp((*r1iter), (*r2iter), t - floor(t)));
		++niter;
	}
	glutPostRedisplay();
	return false;
...
static void writeKeyFrames() {
    ...
    for (int i = 0; i < g_keyFrames.size(); i++)
    {
